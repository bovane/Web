...
在head标签中加入`<style>`标签，<font color=blue>对多个标签进行统一修改，范围针对此页面。</font>
...
<font color=red>CSS的选择器分为两大类：基本选择题和扩展选择器。</font>
...
<font color=red>针对某一个特定的标签来使用，只能使用一次</font>。css中的ID选择器以”#”来定义。
...
比如，我们==可以同时让标签选择器和id选择器作用于同一个标签==。如下：
...
<!-- 类选择器将CSS封装成一个类，然后在任何标签中均可加入 class属性应用样式-->
...
<font color=blue>类选择器的使用，能够决定一个人的css水平。</font>

比如，我们现在要做下面这样一个页面：

![](http://img.smyhvae.com/20170711_1639.png)

正确的思路，就是用**所谓“公共类”的思路，就是我们类就是提供“公共服务”，比如有绿、大、线，一旦携带这个类名，就有相应的样式变化。**对应css里的代码如下：
...
上图所示，<font color=red>css和js都在用同一个id，会出现不好沟通的情况。</font>
...
- 标签选择器针对的是页面上的一类标签。
- ID选择器是只针对特定的标签(一个)，<font color=blue>ID是此标签在此页面上的唯一标识。</font>
- 类选择器可以被多种标签使用。
...
<font color=red>定义交集选择器的时候，两个选择器之间紧密相连。要求同时满足多个条件</font>一般是以标签名开头，比如`div.haha`，再比如`p.special`。
...
<font color=red>选择的元素要求同时满足两个条件：必须是h3标签，然后必须是special标签。</font>
